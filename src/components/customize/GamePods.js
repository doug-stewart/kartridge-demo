...
const GamePods = () => {
  ...
  const reorderPods = (pods) =>
    podUpdater({ type: 'SET_PODS', data: pods.items });
  ...
  const setLayout = (pod, layout) =>
    podUpdater({ type: 'SET_LAYOUT', pod: pod, layout: layout });

  const setImages = (pod, images) =>
    podUpdater({ type: 'SET_IMAGES', pod: pod, images: images });

  const addImage = (pod, data) =>
    podUpdater({ type: 'ADD_IMAGE', pod: pod, image: data });

  const removeImage = (pod, image) =>
    podUpdater({ type: 'REMOVE_IMAGE', pod: pod, image: image });

  const setText = (pod, text) =>
    podUpdater({ type: 'SET_TEXT', pod: pod, text: text });

  const setTrailer = (pod, url) =>
    podUpdater({ type: 'SET_TRAILER', pod: pod, url: url });
  ...
  return (
    <div className="c-feat-pg__sections" ref={podArea}>
      <div
        className={cx('c-feat-pg__sections-inner', 'gp-body', {
          'u-custom--dragging': dragging,
        })}>
        <span className={cx('c-key__group', 'u-custom__sections-keys')}>
          <span className={cx('c-key', 'c-key--d')}>
            <span className="c-key__color"> </span>
            <span className="c-key__label">D</span>
          </span>
          <span className={cx('c-key', 'c-key--e')}>
            <span className="c-key__color">Text</span>
            <span className="c-key__label">E</span>
          </span>
          <span className={cx('c-key', 'c-key--f')}>
        ...
        <div className="c-media__add">
          ...
          {kind !== '' && (
            <button
              className={cx('c-media__add-btn', 'btn--red', 'btn--m')}
              onClick={addPod}>
              Add
            </button>
          )}
        ...
        {podsState.state.length > 0 ? (
          <ReactSortable
            className={cx('u-custom__gallery', 'c-media')}
          ...
          <div className={cx('u-custom__gallery', 'c-media')}>
            <div className="u-custom__empty-gallery">
              <div className="gp-media__trailer" />
              <div className={cx('gp-media__text', 'c-local-reset')}>
                <p />
                <p />
                <p />
                <p />
              ...
              <div>
                <div className={cx('c-media__artwork', 'c-media__artwork--x2')}>
