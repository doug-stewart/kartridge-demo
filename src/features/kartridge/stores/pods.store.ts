...
import type { GalleryPod, Pod, Pods } from '../types';

const changeIndex = (input: Pods, index: number, offset: number) => {
    const array = [...input];
    const currentIndex = array.findIndex((item) => item.id === index);
    const targetIndex = currentIndex + offset;
    const item = array.splice(currentIndex, 1).at(0) as Pod;
    array.splice(targetIndex, 0, item);
    return array;
};

export const addImage = (images: GalleryPod['images'], image: string) => {
    images.push({ id: images.length + 1, image });
    return images;
};

export const removeImage = (images: GalleryPod['images'], id: number) => {
...
export const podsStore = createStore({
    ...
    on: {
        set: (_, event: { pods: Pods }) => ({ pods: event.pods }),
        add: (context, event: { pod: Pod }) => ({
