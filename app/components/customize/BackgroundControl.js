...
const BackgroundControl = ({ background, setBackground }) => {
  ...
  return (
    <fieldset className="u-custom__bg-ctrl c-bgctrl">
      <h2 className="c-bgctrl__title">
        Set your background artwork
      </h2>
      <div className="c-bgctrl__option-image">
        <div className={`c-bgctrl__label ${background.name !== '' ? 'checked' : null}`}>
          <span className="c-bgctrl__txt">
            Upload a video or image
          </span>
          <em className="c-bgctrl__note">
            Minimum 720p. JPG, GIF, PNG, or MP4 recomended.
          </em>
          <Dropzone classes="c-upload has-preview" returner={updateBackground}>
            {background.name === '' ?
              ...
              :
              <span className="c-upload__info">
                <strong className="c-upload__title">
                  {background.name}
                </strong>
